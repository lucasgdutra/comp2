...
2.1. O usuário deve receber uma mensagem se houver erro ao abrir/criar um arquivo ou ao alocar memória.

2.2. Mensagens quando a busca não retorna resultado: Em um arquivo com nomes de pessoas, por exemplo, o usuário solicita uma busca por um nome que não existe no arquivo.
Neste caso, uma mensagem deve avisar o usuário que a busca não teve resultados.
...
4.1. Crie um arquivo novo (por exemplo, copia.txt);

4.2. Grave em copia.txt tudo que deve ser mantido;

4.3. Delete o arquivo original (função remove);

4.4. Renomeie copia.txt para o nome do arquivo original (função rename).

4.5. Lembre-se de desalocar memória e fechar os arquivos abertos. Não fazer isso penalizará a pontuação da dupla
